  ...
  convexHull = (e) => {
    ...
    for (let i = 1; i < n; i++) {
      if (i == n - 1 || this.orientation(p1, points[i], p2) != -1) {
        ...
        while (
          up.length >= 2 &&
          this.orientation(up[up.length - 2], up[up.length - 1], points[i]) ==
            -1
        ) {
          up.pop();
          animate.push(JSON.parse(JSON.stringify(up)));
      ...
      if (i == n - 1 || this.orientation(p1, points[i], p2) != 1) {
        ...
        while (
          lo.length >= 2 &&
          this.orientation(lo[lo.length - 2], lo[lo.length - 1], points[i]) == 1
        ) {
          lo.pop();
          animate.push(JSON.parse(JSON.stringify(lo)));
        }
          ...
          <div className="collapse navbar-collapse" id="navbarSupportedContent">
            <ul className="navbar-nav mr-auto">
              ...
              <li className="nav-item">
                ...
                >
                  Random Points <span className="sr-only">(current)</span>
                </a>
              </li>
              <li className="nav-item">
